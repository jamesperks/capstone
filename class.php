...
function genTestForm ($name=NULL, $location=NULL, $day=NULL, $startHour=NULL, $startMinute=NULL, $startAMPM=NULL, $endHour=NULL, $endMinute=NULL, $endAMPM=NULL) {
    if($name===NULL && $location===NULL && $day===NULL && $startHour===NULL && $startMinute===NULL && $startAMPM===NULL && $endHour===NULL && $endMinute===NULL && $endAMPM===NULL) {
        ...
        return '<form role="form" class="addclass-form" method="post" action="class.php'.((isset($_GET['id'])) ? '?id='.$_GET['id']:NULL).'">
                ...
                <div class="clearfix"></div>
                <div class="form-group" style="margin-top:20px;">
                    <input type="reset" class="btn btn-danger btn-lg" value="Clear" /> '.((isset($_GET['id'])) ? '<a href="hours.php?id='.$_GET['id'].'" class="btn btn-info btn-lg">Study Hours</a>':NULL).' <input type="submit" name="submit" class="btn btn-primary btn-lg pull-right" value="'.((isset($_GET['id'])) ? 'Update':'Add').' Class" />
                </div>
            </form>';
    }
}
if($_POST['submit']) {
    ...
    if(empty($_POST['name'])) {
        ...
        $errors[] = "Please select a class day";
    } else {
            ...
            if(empty($_POST['endHour']) || empty($_POST['endMinute']) || empty($_POST['endAMPM'])) {
                ...
                if($_POST['endHour'] > 12 || $_POST['endHour'] < 1 || $_POST['endMinute'] > 59 || $_POST['endMinute'] < 0) {
                    ...
                    if($startTime > $endTime) {
                        $errors[] = "Your class ends before it starts!";
                    }
                }
            }
        }
    ...
    if(!empty($errors)) {
        ...
        $output .= "</ul>";
        $output .= genTestForm($_POST['name'],$_POST['location'],$_POST['day'],$_POST['startHour'],$_POST['startMinute'],$_POST['startAMPM'],$_POST['endHour'],$_POST['endMinute'],$_POST['endAMPM']);
    } else {
       try {
